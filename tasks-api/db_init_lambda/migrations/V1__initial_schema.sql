...
CREATE TABLE users (
    ...
    profile_type VARCHAR(10) NOT NULL CHECK (profile_type IN ('admin', 'employee')),
...
GROUP BY 
    u.user_id, u.cognito_user_id, u.fname, u.lname;
...
FROM 
    users u
LEFT JOIN 
    tasks t ON u.user_id = t.assigned_to OR u.user_id = t.source
WHERE 
